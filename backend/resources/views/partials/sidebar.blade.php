<aside id="sidebar"
    class="fixed inset-y-0 left-0 z-50 w-64 bg-surface border-r border-border transition-transform duration-300 ease-in-out -translate-x-full md:translate-x-0 md:static md:inset-0">
    <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 px-3 py-6 space-y-1 overflow-y-auto custom-scrollbar">
            ...
            <a href="{{ route('dashboard') }}"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('dashboard') ? 'bg-accent text-white' : 'text-zinc-400 hover:text-white hover:bg-white/5' }} group transition-all">
                <i data-lucide="layout-grid" class="w-5 h-5"></i>
                <span class="text-sm font-medium">Dashboard</span>
            </a>
            <a href="{{ route('links.index') }}"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('links.index') ? 'bg-accent text-white' : 'text-zinc-400 hover:text-white hover:bg-white/5' }} group transition-all">
